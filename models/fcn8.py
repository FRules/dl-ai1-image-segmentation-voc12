...
class Model(AbstractModel):
    ...
    def get_model(self, **kwargs) -> keras.Model:
        ...
        # Encoder - Load VGG16
        encoder = VGG16(weights=kwargs['vgg_weights'] if 'vgg_weights' in kwargs else None, input_tensor=img_input,
        ...
        # Decoder - fcn32
        conv10 = Conv2D(filters=N_CLASSES, kernel_size=(1, 1), activation='relu', padding='same', name='block10_conv1')(
            pool3)
        conv11 = Conv2D(filters=N_CLASSES, kernel_size=(1, 1), activation='relu', padding='same', name='block10_conv2')(
            pool4)
        deconv2 = Conv2DTranspose(filters=N_CLASSES, kernel_size=(1, 1), strides=(2, 2), activation='relu',
                                  padding='same', name='block10_deconv1', use_bias=False)(conv11)
        deconv3 = Conv2DTranspose(filters=N_CLASSES, kernel_size=(1, 1), strides=(4, 4), activation='relu',
                                  padding='same', name='block10_deconv2', use_bias=False)(conv7)
        add2 = Add(name='block10_add2')([conv10, deconv2, deconv3])
        fcn32 = Conv2DTranspose(N_CLASSES, kernel_size=(1, 1), strides=(8, 8), padding='valid', name='block10_deconv3',
                                use_bias=False)(add2)
